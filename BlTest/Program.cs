        ...
        private static void Create(string entityName)
        {
            ...
            switch (entityName)
            {
                ...
                case "AssignmentSubMenu":
                    ...
                    s_bl.Assignment?.Create(assignment);
                    break;
            }
        }
        ...
        private static void ReadAll(string entityName)
        {
            switch (entityName)
            {
                ...
                case "CallSubMenu":
                    ...
                    break;
                case "AssignmentSubMenu":
                    var assignments = s_bl.Assignment?.ReadAll();
        ...
        private static void Update(string entityName)
        {
            ...
            switch (entityName)
            {
                case "VolunteerSubMenu":
                    ...
                    s_bl.Volunteer?.Update(updatedVolunteer);
                ...
                case "AssignmentSubMenu":
                    ...
                    s_bl.Assignment?.Update(updatedAssignment);
                ...
                try
                {
                    switch (choice)
                    {
                        case ConfigSubmenu.AdvanceClockByMinute:
                            s_bl.Config.Clock = s_bl.Config.Clock.AddMinutes(1);
                            break;

                        case ConfigSubmenu.AdvanceClockByHour:
                            s_bl.Config.Clock = s_bl.Config.Clock.AddHours(1);
                            break;

                        case ConfigSubmenu.AdvanceClockByDay:
                            s_bl.Config.Clock = s_bl.Config.Clock.AddDays(1);
                            break;

                        case ConfigSubmenu.AdvanceClockByMonth:
                            s_bl.Config.Clock = s_bl.Config.Clock.AddMonths(1);
                            break;

                        case ConfigSubmenu.AdvanceClockByYear:
                            s_bl.Config.Clock = s_bl.Config.Clock.AddYears(1);
                            break;

                        case ConfigSubmenu.DisplayClock:
                            Console.WriteLine($"Current Clock: {s_bl.Config.Clock}");
                            break;
                        ...
                        case ConfigSubmenu.ChangeClockOrRiskRange:
                            ...
                            s_bl.Config.RiskRange = TimeSpan.FromHours(riskHours);
                            break;

                        case ConfigSubmenu.DisplayConfigVar:
                            Console.WriteLine($"Clock: {s_bl.Config.Clock}");
                            Console.WriteLine($"Risk Range: {s_bl.Config.RiskRange}");
                            break;
